...
import com.example.annotation.CKeep;
...
public class Processor extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        messager.printMessage(Diagnostic.Kind.NOTE, "================================================> printMessage1:");
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        Messager messager = processingEnv.getMessager();
        messager.printMessage(Diagnostic.Kind.NOTE, "================================================> printMessage2:");
        if (!roundEnv.processingOver()) {
            buildAnnotatedElement(roundEnv, CBindView.class);
            buildAnnotatedElement(roundEnv, COnClick.class);
        }else {
            for (Map.Entry<String, List<Element>> entry : annotationClassMap.entrySet()) {
                ...
                */
                ...
                for (VariableElement variableElement : ElementFilter.fieldsIn(entry.getValue())) {
                    CBindView bindView = variableElement.getAnnotation(CBindView.class);
                    if (bindView != null) {
                ...
                *   private void bindOnClicks(final MainActivity activity) {
                ...
                for (ExecutableElement executableElement : ElementFilter.methodsIn(entry.getValue())) {
                    COnClick onClick = executableElement.getAnnotation(COnClick.class);
                    if (onClick != null) {
    ...
    public Set<String> getSupportedAnnotationTypes() {
        return new TreeSet<>(Arrays.asList(
                CBindView.class.getCanonicalName(),
                COnClick.class.getCanonicalName(),
                CKeep.class.getCanonicalName()));
    }
